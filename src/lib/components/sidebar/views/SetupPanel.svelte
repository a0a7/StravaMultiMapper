			...
			{#if $page.data.session?.user}
				{#if $page.data.session.user?.image && $page.data.session.user?.name}
					<Avatar.Root class="rounded-xl">
						<Avatar.Image src="{$page.data.session.user.image}" alt="{$page.data.session.user.name} Profile picture"/>
						<Avatar.Fallback></Avatar.Fallback>
					</Avatar.Root>
					<p class="mx-3 leading-[1.125]">
						Connected&nbsp;to&nbsp;Strava&nbsp;as<br><b>{$page.data.session.user.name.replace(/ /g, '\xa0')}</b>.
					</p>
				{:else}
					<p>
						Connected to Strava, but with profile information missing. Something probably went wrong somewhere.
					</p>
				{/if}
			{/if}
		</div>
		...
		<div class="flex flex-col justify-center items-center w-full mt-2">
			<div class="pb-4 text-center">
				<p class="mt-3 font-bold text-xl">
					Now fetching your Strava activities.
				</p>
